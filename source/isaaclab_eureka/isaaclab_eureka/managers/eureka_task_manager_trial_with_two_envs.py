...
def _get_rewards(self):
    if self.debug:
        print("In eureka generated get_rewards")
    rewards_oracle = self._get_rewards_oracle()
    if self.debug:
        print("rewards_oracle data type:")
        print(rewards_oracle)
    ...
    if self.debug:
        print(self._eureka_episode_sums.keys())
        print("rewards_eureka")
        print(rewards_eureka.size())
...
class EurekaTaskManager:
    ...
    def _worker(self, idx: int, rewards_queue: multiprocessing.Queue):
        ...
        while not self.termination_event.is_set():
            # Put it here to avoid env generation error
            # the two should be shared for both
            result = {"success": True}  
            task_name = "task"
            self._create_environment(task_name=task_name)
            if self._idx == 0 and not hasattr(self, "_observation_string"):
                self._observation_string = inspect.getsource(self._env._get_observations)
            ...
            if self._debug:
                print(f"reward_func_string {reward_func_string}")
            if isinstance(reward_func_string, str) and reward_func_string.startswith("def _get_rewards_eureka(self)"):
                ...
                try:
                    ...
                    with context:
                        ...
                        self._run_training(task_name=task_name)
                    if self._replay_task is not None:
                        result["log_dir"] = self._log_dir
                except Exception as e:
                    result = {"success": False, "exception": str(e)}
                    print(traceback.format_exc())
                    self.error_flag = True
                    print("Error during pipeline(Probably )")
            else:
            ...
            if self.error_flag:
                ...
                self._env.sim.clear()
                self.error_flag = False
            task_name = "replay_task"

            # ignore the get observation string part: replay doesn't need observation at all
            # Insert the reward function into the environment and run the training
            ...
            if isinstance(reward_func_string, str) and reward_func_string.startswith("def _get_rewards_eureka(self)"):
                ...
                try:
                    ...
                    with context:
                        # Run training and send result to main process
                        self._run_training(task_name=task_name)
                        result["replay_log_dir"] = self._replay_log_dir
                except Exception as e:
                    result = {"success": False, "exception": str(e)}
                    print(traceback.format_exc())
                    self.error_flag = True
                ...
                result = {
                    "success": False,
                    "exception": (
                        "For replay env, the reward function must be a string that starts with 'def _get_rewards_eureka(self)'."
                    ),
                }
            if self.error_flag:
                self._env.close()
    ...
    def _simulation_setup(self): # called once only
    ...
    def _create_environment(self, task_name):
        ...
        if task_name == "task":
            ...
            self._env = gym.make(self._task, cfg=env_cfg).unwrapped

        elif task_name == "replay_task":
            env_cfg: DirectRLEnvCfg = parse_env_cfg(self._replay_task)
            env_cfg.sim.device = self._device
            env_cfg.seed = self._env_seed
            self._env = gym.make(self._replay_task, cfg=env_cfg).unwrapped
    ...
    def _prepare_eureka_environment(self, get_rewards_method_as_string: str):
        ...
        if not hasattr(env, "_get_rewards_eureka"):
            if self._debug:
                print("current env has no _get_rewards_eureka")
    ...
    def _run_training(self, task_name, framework: Literal["rsl_rl", "rl_games"] = "rsl_rl"):
        """Run the training of the task."""
        from isaaclab_tasks.utils.parse_cfg import load_cfg_from_registry
        if self._replay_task is not None and task_name == "replay_task":
            # set up log dir 
            ...
            # set up runner
            # TODO: printing part is done by setting up env? Maybe I don't have to print
            from isaaclab_eureka.managers import ReplayRunner
            replayrunner = ReplayRunner(env=self._env,log_dir=self._replay_log_dir) # TODO: log dir
            self._env._reset_idx_original(None) # call the original to avoid the bug?
            while self._simulation_app.is_running():
                _, rewards, _, _, _ = replayrunner.step(self._env.actions)
                if self._env.common_step_counter >= self._env.max_len - 1: # only for envs with...
            ...
            elif self._rl_library == "rl_games": # No changes for rl_games for now
